...
`lag_seconds` 会在 0 到约 2000 秒之间**周期性波动**：模型每 30 分钟重算一次，
刚跑完时只有一两分钟，下一轮开始前接近 30 分钟。这是正常的锯齿，不是故障。
`fin_daily` 的滞后按天计（当天的行要到次日才稳定），属于预期。
...
| `v_chip_hourly` | **每芯片每小时** | 1.2 万/天 | **主力宽表**：分子与 in-VM 分母同在一行 |
| `v_capacity_daily` | 每预留每天 | 2/天 | 已付费产能，`global_*` 指标的分母 |
| `v_utilization_daily` | 天 | 1/天 | 已算好的四个比率，含覆盖率闸门 |
| `v_metric_export` | 天 × 指标 | 11/天 | 长格式，每行自带计算公式 |
| `v_job` | 每 job | 2.6 万 | job 清单、芯片数、goodput、TensorBoard 地址 |
| `v_incident` | 每次事故 | 1,500 | 维护、抢占、自动修复的生命周期 |
| `v_event` | 每条事件 | **3,067 万** | 原始事件流，**必须带时间条件** |
| `v_step` | 每训练步 | **100 万** | loss、梯度、TFLOP/s、掉队比，**必须带时间条件** |
...
| 视图 | 延迟范围 | 原因 |
|---|---|---|
| `v_chip_hourly` | 2–32 分钟 | 模型每 30 分钟重算，刚跑完时最小 |
